...
def ast_to_infix(node):
    ...
    if op=='div': return f"({ast_to_infix(a[0])} / {ast_to_infix(a[1])})"
    if op=='sqrt': return f"sqrt({ast_to_infix(a[0])})"
    if op=='log': return f"log({ast_to_infix(a[0])})"
    if op=='abs': return f"abs({ast_to_infix(a[0])})"
    if op=='neg': return f"(-{ast_to_infix(a[0])})"
    if op=='inv': return f"(1/({ast_to_infix(a[0])}))"
    if op=='max': return f"max({', '.join(ast_to_infix(c) for c in a)})"
    if op=='min': return f"min({', '.join(ast_to_infix(c) for c in a)})"
    if op=='sin': return f"sin({ast_to_infix(a[0])})"
    if op=='cos': return f"cos({ast_to_infix(a[0])})"
    if op=='tan': return f"tan({ast_to_infix(a[0])})"
